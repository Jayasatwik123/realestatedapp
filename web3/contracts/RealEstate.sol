...
,string memory _images,string memory _propertyAddress,
  string memory _description) external returns(uint256){
Property storage property=properties[productId];
...
function updatePrice(address owner,uint256 productId,uint256 price) external  returns (string memory){
    Property storage property=properties[productId];
    require(property.owner==owner,"u r not owner");
    property.price=price;
...
function addreview(uint256 productId,uint256 rating,string calldata comment,address user) external{
  require(rating>=1&&rating<=5,"rating must ebe bwtween 1 and 5");
  Property storage property=properties[productId];

  property.reviweers.push(user);
...
function getHighestratedProduct() external view returns(uint256){
  ...
  for(uint256 i=0;i<reviewsCouter;i++){
